...
def gen_all_combinations(l:List[int]) -> List[str]:
    '''
    Generating every permutation possible for every row. Returns a list of permutations as strings.
    ...
    for block in l:
        block_combinations = nonoblock.create_combinations(block, size)
        block_combinations = [nonoblock.block2string(c) for c in block_combinations]        
...
def invert(board:List[list], join=True) -> List[list]:
    '''
    Turns the rows into columns.
    '''
    columns = []
    for i in range(SIZE):
        # col = ''.join([x[i] for x in board])
...
def deduce(combinations:Dict[str, List[str]], guaranteed:List[str], rows=True, r:int=0) -> List[str]:
    ...
    if puzzle_solved:
        ...
        print('-' * (SIZE+4))
        for block in solution:  print(f'| {block} |')
        print('-' * (SIZE+4))
